<template>
	<AdminLayout>
		<AdminIndexLayout>
			<AdminIndexHeader>
				<AdminIndexHeaderTitle :title="'Все кафедры'"/>
				<AdminIndexSearch/>
				<AdminIndexFilter/>
			</AdminIndexHeader>
			<AdminIndexTable>
				<AdminIndexTableHead>
					...
					<AdminIndexTableRow :name="'Факультет'"/>
					<AdminIndexTableRow :name="'Дата создания'"/>
					<th scope="col" class="px-6 py-3 text-right"></th>
				</AdminIndexTableHead>
				<AdminIndexTableBody>
					<template v-for="department in departments.data">
						<tr>
							<AdminIndexTableCell :value="department.id"/>
							<AdminIndexTableCell :value="department.title" :text-limit-value="20"/>
							<AdminIndexTableCell :value="department.faculty"/>
							<AdminIndexTableCell :value="department.created_at"/>
							<AdminIndexTableButtonGroup>
								<AdminIndexTableDropdownButton :title="'Действия'">
									<AdminIndexTableDropdownElement :title="'Обновить'" :link="route('admin.department.edit', department.id)" :method="'get'" />
									<AdminIndexTableDropdownElement :title="'Удалить'" :link="route('admin.department.destroy', department.id)" :method="'delete'" />
			...
			<AdminIndexFooter>
				<AdminIndexSelectPerPage :item-length="this.departments.data.length" />
				<AdminIndexPaginate v-if="this.departments.links" :links="this.departments.links"/>
			</AdminIndexFooter>
		</AdminIndexLayout>
	</AdminLayout>
</template>
