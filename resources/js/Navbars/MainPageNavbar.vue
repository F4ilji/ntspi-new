<template>
		...
		<nav class="max-w-screen-xl w-full mx-auto px-4 py-3" aria-label="Global">
			<div class="relative md:flex md:items-center md:justify-between">
				<div class="flex items-center justify-between">
					...
					<div class="md:hidden">
						<button :class="underSliderHeader ? 'text-black' : 'text-white'" @click.prevent="toggleHeaderFilter"
										type="button"
				...
				<div id="navbar-collapse-with-animation"
					...
					<div
						...
						<div
							...
							<template v-for="section in this.sections.data" :key="section.id">
								<div
									...
									<button type="button" :class="!underSliderHeader ? 'text-white' : 'text-black'"
													class="active:text-blue-600 duration-300 flex items-center w-full hover:text-gray-300 font-medium dark:text-gray-200 dark:hover:text-gray-500 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
										{{ section.title }}
							...
							<a class="hover:opacity-70 py-3 cursor-pointer" data-hs-overlay="#hs-basic-modal">
								<svg :class="!underSliderHeader ? 'text-white' : 'text-black'" xmlns="http://www.w3.org/2000/svg"
										 fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
										 class="w-6 h-6 duration-300 md:block hidden">
									<path stroke-linecap="round" stroke-linejoin="round"
	...
	<div style="z-index: 1000" id="hs-basic-modal"
			 class="hs-overlay hs-overlay-backdrop-open:bg-gray-200/60 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 hidden size-full fixed top-[121px] start-0 opacity-0 overflow-x-hidden transition-all overflow-y-auto pointer-events-none">
		<div class="sm:max-w-xl sm:w-full m-3 sm:mx-auto ">
			<div
					class="flex flex-col bg-white border border-[0.5px] border-gray-300 shadow-sm rounded-xl pointer-events-auto">
				<div class="overflow-y-auto">
					<div class="w-full space-y-3">
						<ClientGlobalSearch></ClientGlobalSearch>
					</div>
				</div>
			</div>
		</div>
...
export default {
	...
	methods: {
		...
		handleScroll() {
			this.underSliderHeader = window.pageYOffset > ((window.innerHeight * 100) / 100) - 50
			this.scrollPosition = window.pageYOffset
			this.headerFilter = this.scrollPosition > 90
		},
		toggleHeaderFilter() {
			if (this.scrollPosition < 90) {
				this.headerFilter = !this.headerFilter
			}
		},
